...
def import_project(runtime=None, params=None):
    ...
    verbose = bool(params.get("import_verbose", False) or params.get("verbose", False))
    ...
    try:
        ...
        results = find_all_changes(base_dir, projects_obj, export_xml=export_xml, verbose=verbose)
        ...
        if not to_import:
            ...
            summary = "Updated: 0, Created: 0, Moved: 0, Deleted: 0, Failed: 0 (Identical: " + str(unchanged_count) + ")"
            if verbose:
                print("No changes to import.\nAll " + str(unchanged_count) + " objects are in sync.")
            else:
                print("Import complete | " + summary + " | Time: {:.2f}s".format(elapsed))
            runtime.ui.info("No changes to import.\nAll " + str(unchanged_count) + " objects are in sync.\nTime: {:.2f}s".format(elapsed))
            return {
                "status": "success",
                "summary": {
                    "updated": 0,
        ...
        finalize_sync_operation(base_dir, projects_obj, is_import=True, verbose=verbose)
        return {
            "status": "success",
            "summary": {
                "updated": updated,
